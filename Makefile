...
run-container:
	...
	docker build -t dmighty007 -f Dockerfile .
	docker run --env-file $(ENV) -v ./assets/:/dmighty007/assets/ dmighty007
.PHONY: run-container
...
clean:
	...
	rm -rf assets
	rm -f package*.json
	docker rm -f dmighty007 2>/dev/null || true
	docker rmi $(docker images | grep "dmighty007") 2> /dev/null || true
.PHONY: clean